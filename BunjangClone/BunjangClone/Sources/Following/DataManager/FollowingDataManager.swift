...
class FollowingDataManager {
    func getFollowing(viewController: BaseViewController) {
        let keyChain = KeychainSwift()
        if let jwt = keyChain.get("jwtToken") {
            let headers: HTTPHeaders = ["x-access-token" : jwt]
            AF.request("\(Constant.BASE_URL)/myshop/following", method: .get, headers: headers).validate().responseDecodable(of: FollowingResponse.self) { response in
                switch response.result {
                case .success(let response):
                    let message = response.message
                    if response.isSuccess {
                        print(message)
                        viewController.setFollowing(response.result)
                    } else {
